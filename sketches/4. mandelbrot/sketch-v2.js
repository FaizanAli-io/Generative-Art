...
let colorMode = 2; // Fire color scheme
let colorCycle = 0;
let resolutionScale = 2;
let frameSkip = 0;
let colorTable = [];

// Supersampling factor for smoother visuals
let supersampleFactor = 4; // Increase for smoother visuals
...
function initColorTable() {
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    // Rainbow color scheme (not used but pre-calculated for potential future use)
...
function draw() {
  ...
  if (frameCount % (frameSkip + 1) === 0) {
    ...
    // Optimization: Render at a higher resolution for supersampling
    let scaledWidth = width * supersampleFactor;
    let scaledHeight = height * supersampleFactor;
    ...
    for (let x = 0; x < scaledWidth; x++) {
      for (let y = 0; y < scaledHeight; y++) {
        ...
        // Smooth the transition between iteration counts
        norm = smoothstep(norm);

        let r, g, b2;

        // Fire color scheme (colorMode 2)
        ...
        for (let dx = 0; dx < supersampleFactor; dx++) {
          for (let dy = 0; dy < supersampleFactor; dy++) {
            if (x * supersampleFactor + dx < width && y * supersampleFactor + dy < height) {
              let pix = (x * supersampleFactor + dx + (y * supersampleFactor + dy) * width) * 4;
              pixels[pix + 0] = r;
              pixels[pix + 1] = g;
              pixels[pix + 2] = b2;
              pixels[pix + 3] = 255;
